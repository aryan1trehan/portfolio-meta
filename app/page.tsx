...
export default function Home() {
  return (
    <main className="bg-[#000000] text-white">
      ...
      {/* SECTION 01 — HERO */}
      <Card className="w-full min-h-screen bg-[#000000] relative overflow-hidden rounded-none border-none">
        ...
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none"
          style={{ backgroundImage: 'linear-gradient(45deg, #6D001A 1px, transparent 1px), linear-gradient(-45deg, #6D001A 1px, transparent 1px)', backgroundSize: '80px 80px' }} />
        ...
        <div className="flex h-screen">
          <div className="flex-1 p-8 md:p-16 relative z-10 flex flex-col justify-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-16">
              <a href="#contact"
                className="animate-pulse-cta inline-block px-8 py-4 bg-[#6D001A] hover:bg-[#8B0022] text-white font-georgia font-bold text-base rounded-none transition-all duration-200 text-center">
                Start with a Free Audit
              </a>
              <a href="#work"
                className="inline-block px-8 py-4 border border-[#3D0010] hover:border-[#6D001A]/50 text-white/70 hover:text-white font-inter text-base transition-all duration-200 text-center">
                View our work  ↓
              </a>
            </div>
            {/* Trust strip */}
            <div className="flex flex-wrap gap-3">
              {['₹78Cr+ in managed spend', 'Up to 7x ROAS', '25+ brands · 7 continents', 'boAt · AstroTalk · Umaid Palace'].map((pill) => (
                <span key={pill} className="font-inter text-xs text-white/50 border border-[#3D0010] px-4 py-2 tracking-wide">
                  {pill}
                </span>
              ))}
            </div>
      ...
      <FlowArt aria-label="Enhanccee Story">
        ...
        <FlowSection aria-label="The process" style={{ backgroundColor: '#000000', color: '#fff' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-0 relative">
            {/* Connecting line */}
            <div className="hidden md:block absolute top-8 left-0 right-0 h-px bg-[#6D001A]/20 z-0" />
            {[
