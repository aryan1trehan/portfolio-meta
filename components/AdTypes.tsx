    ...
    <div>
      ...
      <div className="hidden md:block">
          ...
          {tabs.map((tab, i) => (
            <button
              ...
              className={`font-inter text-xs uppercase tracking-widest px-5 py-2.5 transition-all duration-200 ${
                active === i
                  ? "bg-[#545677] text-white"
                  : "border border-[#0D3560] text-[#E2E8F4]/60 hover:border-[#545677]/40 hover:text-[#545677]"
              }`}
            >
              {tab.label}
            </button>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div>
              <h3 className="font-georgia text-2xl text-[#E2E8F4] mb-3">{tabs[active].headline}</h3>
              <p className="font-inter text-sm text-[#E2E8F4]/60 leading-relaxed">{tabs[active].body}</p>
            </div>
            <ul className="flex flex-col gap-3">
              {tabs[active].bullets.map((b, i) => (
                <li key={i} className="flex items-start gap-3 font-inter text-sm text-[#E2E8F4]">
                  <span className="text-[#545677] mt-1 shrink-0">—</span>
                  {b}
                </li>
              ))}
            </ul>
      ...
      <div className="md:hidden flex flex-col gap-2">
        {tabs.map((tab, i) => (
          <div key={i} className="border border-[#0D3560]">
            <button
              onClick={() => setOpenAccordion(openAccordion === i ? null : i)}
              className="w-full flex items-center justify-between px-5 py-4 font-inter text-sm font-semibold text-[#E2E8F4] uppercase tracking-widest"
            >
              {tab.label}
              <span className="text-[#545677] text-lg">{openAccordion === i ? "−" : "+"}</span>
            </button>
            <AnimatePresence>
              {openAccordion === i && (
                <motion.div
                ...
                >
                  <div className="px-5 pb-5">
                    <p className="font-georgia text-lg text-[#E2E8F4] mb-2">{tab.headline}</p>
                    <p className="font-inter text-sm text-[#E2E8F4]/60 mb-4 leading-relaxed">{tab.body}</p>
                    <ul className="flex flex-col gap-2">
                      {tab.bullets.map((b, j) => (
                        <li key={j} className="flex items-start gap-3 font-inter text-sm text-[#E2E8F4]">
                          <span className="text-[#545677] shrink-0">—</span>
                          {b}
                        </li>
                      ))}
                    </ul>
